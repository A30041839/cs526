...
struct super_block_t {
  ...
  uint32_t compute_checksum() {
    ...
    for (int i = 1; i < 1024; ++i) {
      uint32_t* ptr = (uint32_t*)this + i * 4;
      checksum ^= *ptr;
    }
...
class server_log {
  ...
  public:
    ...
    void write_super_block() {
      int fd = open(log, O_RDWR);
      void* addr = mmap(NULL, BLOCK_SIZE, PROT_WRITE, MAP_SHARED, fd, 0);
      memcpy(addr, (void*)&super_block, BLOCK_SIZE);
      msync(addr, BLOCK_SIZE, MS_SYNC);
      munmap(addr, BLOCK_SIZE);
      close(fd);
    }

    void write_log_block(uint32_t offset) {
      int fd = open(log, O_RDWR);
      void* addr = mmap(NULL, BLOCK_SIZE, PROT_WRITE, MAP_SHARED, fd, offset * BLOCK_SIZE);
      memcpy(addr, (void*)&cur_block, BLOCK_SIZE);
      msync(addr, BLOCK_SIZE, MS_SYNC);
      munmap(addr, BLOCK_SIZE);
      close(fd);
    }

    void write_checkpt_block(uint32_t offset) {
      int fd = open(log, O_RDWR);
      void* addr = mmap(NULL, BLOCK_SIZE, PROT_WRITE, MAP_SHARED, fd, offset * BLOCK_SIZE);
      memcpy(addr, (void*)&checkpt_block, BLOCK_SIZE);
      msync(addr, BLOCK_SIZE, MS_SYNC);
      munmap(addr, BLOCK_SIZE);
      close(fd);
    }
    ...
    void recover_from_checkpoint() {
      ...
      for (uint32_t i = 0; i < super_block.checkpoint_size; ++i) {
        read_in_checkpt_block(super_block.log_size + i);
    ...
    void play_log() {
      ...
      for (uint32_t i = log_start; i < log_size; ++i) {
        read_in_log_block(i);
        //check checksum and generation number
        if (cur_block.checksum == cur_block.compute_checksum()
            && cur_block.generation_num == super_block.generation_num) {
          //this log block is valid
          for (uint32_t k = 0; k < cur_block.entry_cnt; ++k) {
            execute_log_entry(&cur_block.log_entry[k]);
          }
        }else {
    ...
    void add_checkpt_entry(uint64_t node1, uint64_t node2) {
      ...
      write_checkpt_block(block_offset);
    }
